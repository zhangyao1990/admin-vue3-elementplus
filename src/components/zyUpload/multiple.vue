<template>
	<div class="zy-upload-multiple">
		<div class="zy-upload-list">
			<ul>
				<li v-for="(file, index) in fileList" :key="index">
					<div v-if="file.status!='success'" class="zy-upload-item" v-loading="true" element-loading-background="rgba(0, 0, 0, 0.5)">
						<el-image class="image" :src="file.tempImg" fit="cover" :z-index="9999"></el-image>
					</div>
					<div v-else class="zy-upload-item">
						<div class="mask">
		...
		<div class="zy-upload-uploader" @click="fileSelect && showfileSelect()">
		...
		<el-dialog title="打开" v-model="fileSelectDialogVisible" :width="880" destroy-on-close>
			<zy-file-select multiple @submit="fileSelectSubmit">
				<template #do>
...
<style scoped>
	...
	.zy-upload-multiple {display: inline-block;}


	.zy-upload-list {display: inline-block;}
	.zy-upload-list li {list-style: none;display: inline-block;width: 120px;height: 120px;margin-right: 10px;margin-bottom:10px;vertical-align: top;box-sizing: border-box;}

	.zy-upload-item {position: relative;width: 100%;height: 100%;}
	.zy-upload-item .mask {display: none;position: absolute;top:0px;right:0px;line-height: 1;z-index: 1;}
	.zy-upload-item .mask span {display: inline-block;width: 25px;height:25px;line-height: 23px;text-align: center;cursor: pointer;color: #fff;}
	.zy-upload-item .mask span i {font-size: 12px;}
	.zy-upload-item .mask .del {background: #F56C6C;}

	.zy-upload-item:hover .mask {display: inline-block;}


	.zy-upload-list .image {width: 100%;height: 100%;}

	.zy-upload-list .image-slot {display: flex;justify-content: center;align-items: center;width: 100%;height: 100%;background: #f5f7fa;color: #909399;}
	.zy-upload-list .image-slot i {font-size: 20px;}

	.zy-upload-uploader {border: 1px dashed #d9d9d9;width: 120px;height: 120px;display: inline-block;vertical-align: top;box-sizing: border-box;}
