...
<template>
	<div class="zyTable" :style="{'height':_height}" ref="zyTableMain" v-loading="loading">
		<div class="zyTable-table">
			<el-table v-bind="$attrs" :data="tableData" :row-key="rowKey" :key="toggleIndex" ref="zyTable" height="100%" @sort-change="sortChange" @filter-change="filterChange">
				<slot></slot>
		...
		<div class="zyTable-page" v-if="!hidePagination&&!hideDo">
			...
			<div class="zyTable-do" v-if="!hideDo">
	...
	export default {
		...
		methods: {
			...
			async getData(){
				...
				this.$refs.zyTable.$el.querySelector('.el-table__body-wrapper').scrollTop = 0
			},
			//分页点击
			paginationChange(){
				this.getData();
			...
			//刷新数据
			refresh(){
				this.$refs.zyTable.clearSelection();
				this.getData();
			},
			//更新数据 合并上一次params
			...
			reload(params, page=1){
				...
				this.$refs.zyTable.clearSelection();
				this.$refs.zyTable.clearSort()
				this.$refs.zyTable.clearFilter()
				this.getData()
			},
			//自定义变化事件
			...
			toggleRowSelection(row, selected){
				this.$refs.zyTable.toggleRowSelection(row, selected)
			},
			toggleAllSelection(){
				this.$refs.zyTable.toggleAllSelection()
			},
			toggleRowExpansion(row, expanded){
				this.$refs.zyTable.toggleRowExpansion(row, expanded)
			},
			setCurrentRow(row){
				this.$refs.zyTable.setCurrentRow(row)
			},
			clearSort(){
				this.$refs.zyTable.clearSort()
			},
			clearFilter(columnKey){
				this.$refs.zyTable.clearFilter(columnKey)
			},
			doLayout(){
				this.$refs.zyTable.doLayout()
			},
			sort(prop, order){
				this.$refs.zyTable.sort(prop, order)
			}
		}
	}
</script>
