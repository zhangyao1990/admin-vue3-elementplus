...
<template>
	<div class="zy-file-select">
		<div class="zy-file-select__side" v-loading="menuLoading">
			<div class="zy-file-select__side-menu">
			...
			<div class="zy-file-select__side-msg" v-if="multiple">
				已选择 <b>{{value.length}}</b> / <b>{{max}}</b> 项
			</div>
		</div>
		<div class="zy-file-select__files" v-loading="listLoading">
			<div class="zy-file-select__top">
				<div class="upload" v-if="!hideUpload">
					<el-upload class="zy-file-select__upload" action="" multiple :show-file-list="false"
			...
			<div class="zy-file-select__list">
				<el-scrollbar ref="scrollbar">
					<el-empty v-if="fileList.length==0 && data.length==0" description="无数据" :image-size="80"></el-empty>
					<div v-for="(file, index) in fileList" :key="index" class="zy-file-select__item">
						<div class="zy-file-select__item__file">
							<div class="zy-file-select__item__upload">
					...
					<div v-for="item in data" :key="item[fileProps.url]" class="zy-file-select__item"
						 :class="{active: value.includes(item[fileProps.url]) }" @click="fnSelect(item)">
						<div class="zy-file-select__item__file">
							<div class="zy-file-select__item__checkbox" v-if="multiple">
								<i class="el-icon-check"></i>
							</div>
							<div class="zy-file-select__item__select" v-else>
								<i class="el-icon-check"></i>
							</div>
							<div class="zy-file-select__item__box"></div>
							<div class="zy-preview-src" @click.stop="fnPreview(item)">
							...
							<div v-else class="item-file item-file-doc">
								...
								<i v-else class="zy-icon-file-list-fill" style="color: #999;"></i>
							</div>
						</div>
			...
			<div class="zy-file-select__pagination">
			...
			<div class="zy-file-select__do">
				<slot name="do"></slot>
				<el-button type="primary" :disabled="value.length<=0" @click="submit">确 定</el-button>
			</div>
			<el-dialog width="600px" center v-model="dialogImgVisible" :title="listImgData.fileName">
				<el-image :src="listImgData.url" fit="contain" class="preview_img" lazy></el-image>
			</el-dialog>
		</div>
	</div>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				value: this.multiple ? [] : '',
				fileList: [],
				accept: this.onlyImage ? "images/gif, images/jpeg, images/png" : "",
				listLoading: false,
				menuLoading: false,
...
<style scoped>
	...
	.zy-file-select__files {
		flex: 1;
	}

	.zy-file-select__list {
		height: 400px;
	}

	.zy-file-select__item {
	...
	.zy-file-select__item__file {
		width: 110px;
		height: 110px;
		position: relative;
	}

	.zy-file-select__item__file .el-image {
		width: 110px;
		height: 110px;
	}

	.zy-file-select__item__box {
		position: absolute;
		top: 0;
		right: 0;
		bottom: 0;
	...
	.zy-preview-src:hover i {
		color: #333333;
	}

	.zy-file-select__item__box::before {
		content: '';
		position: absolute;
		top: 0;
		right: 0;
	...
	.zy-file-select__item:hover .zy-preview-src {
		display: inline-block;
		position: absolute;
		right: 7px;
		top: 4px;
	...
	.zy-file-select__item:hover .zy-file-select__item__box {
		display: block;
		background: #e6e6e6;
		opacity: .4;
	}

	.zy-file-select__item.active .zy-file-select__item__box {
		display: block;
	}

	.zy-file-select__item.active .zy-file-select__item__box::before {
		display: block;
	}

	.zy-file-select__item p {
		margin-top: 10px;
		white-space: nowrap;
	...
	.zy-file-select__item__checkbox {
		position: absolute;
		width: 20px;
		height: 20px;
		top: 7px;
	...
	.zy-file-select__item__checkbox i {
		font-size: 14px;
		color: #fff;
		font-weight: bold;
		display: none;
	...
	.zy-file-select__item__select i {
		font-size: 14px;
		color: #fff;
		font-weight: bold;
	}

	.zy-file-select__item.active .zy-file-select__item__checkbox {
		background: var(--el-color-success);
	}

	.zy-file-select__item.active .zy-file-select__item__checkbox i {
		display: block;
	}

	.zy-file-select__item.active .zy-file-select__item__select {
		display: flex;
	}

	.zy-file-select__item__file .item-file {
		width: 110px;
		height: 110px;
		display: flex;
		flex-direction: column;
	...
	.zy-file-select__item__file .item-file i {
		font-size: 40px;
	}

	.zy-file-select__item__file .item-file.item-file-doc {
		color: #409eff;
	}

	.zy-file-select__item__upload {
		position: absolute;
		top: 0;
		right: 0;
		bottom: 0;
	...
	.zy-file-select__side-menu {
		flex: 1;
	}

	.zy-file-select__side-msg {
		height: 32px;
		line-height: 32px;
	}
	...
	.zy-file-select__upload {
		display: inline-block;
	}

	.zy-file-select__top .tips {
		font-size: 12px;
		margin-left: 10px;
		color: #999;
	}

	.zy-file-select__top .tips i {
		font-size: 14px;
		margin-right: 5px;
	}

	.zy-file-select__pagination {
		margin: 15px 0;
	}
