		...
		<el-row :gutter="15">
			<template v-for="(item, index) in config.formItems" :key="index">
				<el-col :span="item.span || 24" v-if="!hideHandle(item)">
					<zy-title  v-if="item.component=='title'"  :title="item.label"></zy-title>
					<el-form-item v-else :prop="item.name" :rules="rulesHandle(item)">
						...
						<template v-else-if="item.component=='upload'" >
							<el-col v-for="(_item, _index) in item.options.items" :key="_index">
								<el-form-item :prop="_item.name">
									<zy-upload v-model="form[_item.name]" :title="_item.label"></zy-upload>
								</el-form-item>
							</el-col>
						</template>
						<!-- switch -->
						...
						<template v-else-if="item.component=='editor'" >
							<zy-editor v-model="form[item.name]" placeholder="请输入" :height="400"></zy-editor>
						</template>
						<!-- noComponent -->
						<template v-else>
...
<script>
	...
	const zyEditor = defineAsyncComponent(() => import('@/components/zyEditor'))
