<template>
	<el-main>
		<el-alert title="感谢百度Echarts组件, ZYUI经过封装后在VUE里使用更方便而且暴露Echarts所有方法,具体请查看本文件" type="success"
				  style="margin-bottom:20px;"></el-alert>
		<el-button type="primary" style="margin-bottom: 10px;" @click="fnBigdata">大屏数据</el-button>
		<el-row :gutter="15">
			<el-col :lg="8">
...
<script>
	import scEcharts from '@/components/zyEcharts';

	/**
	 * 引入组件 @/components/zyEcharts
	 * 组件内部会自动加载主题 @/components/zyEcharts/echarts-theme-T.js
	 * 支持props包括 height，width，option
	 * 组件export百度Echarts所有方法，使用方式: new zyEcharts[fun]
	 */
	...
	export default {
		...
		methods: {
			fnBigdata () {
				let routeUrl =this.$router.resolve({path: '/vab/bigdata'})
